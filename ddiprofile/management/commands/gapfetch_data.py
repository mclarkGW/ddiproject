...
from ddiprofile.models import gapEpic, gapFeature, gapUserStory, gapLastRefreshed, gapChangeRequest, gapTask

class Command(BaseCommand):
    help = 'Fetch GAPs from Azure DevOps and store them in the database'
    ...
    def handle(self, *args, **kwargs):
                    ...
                    for changerequest_data in changerequests:
                        ...
                        if changerequest_data.get('state') == "Done":
                            ...
                            if not target_date or target_date <= CUTOFF_DATE:
                                print(
                                    f"Skipping GAP ID {changerequest_data.get('id')} because State is 'Done' and TargetDate is not after 1/15/2025"
                                )
                                continue
                        ...
                        change_request_instance, created = gapChangeRequest.objects.update_or_create(
                            id=changerequest_data.get('id'),
                            defaults=defaults
                        )
                        seen_cr_ids.add(change_request_instance.id)
                        print(f"Saving GAP: {changerequest_data.get('id')} | {changerequest_data.get('title')}")

                        # Query associated Features
                        tt_workdescription = changerequest_data.get('tt_workdescription')
                        if not tt_workdescription:
                            print(
                                f"Skipping Feature Query for GAP ID {changerequest_data.get('id')} due to missing tt_workdescription")
                            continue
                        ...
                        if feature_work_items_ids:
                            ...
                            for feature_data in features:
                                ...
                                feature_instance, created = gapFeature.objects.update_or_create(
                                ...
                                if user_stories:
                                    for user_story_data in user_stories:
                                        ...
                                        user_story_instance, _ = gapUserStory.objects.update_or_create(
                                        ...
                                        if task_children:
                                            for task_data in task_children:
                                                ...
                                                task_instance, _ = gapTask.objects.update_or_create(
                                                    id=task_data.get('id'),
                                                    defaults={
        ...
        gapChangeRequest.objects.exclude(id__in=seen_cr_ids).delete()
        gapFeature.objects.exclude(id__in=seen_feature_ids).delete()
        gapUserStory.objects.exclude(id__in=seen_userstory_ids).delete()
        gapTask.objects.exclude(id__in=seen_task_ids).delete()

        # Update the LastRefreshed model
        now_utc = datetime.now(pytz.utc)
        eastern = pytz.timezone('US/Eastern')
        now = now_utc.astimezone(eastern)
        gapLastRefreshed.objects.update_or_create(
            id=1,
            defaults={
                'title': now_utc.strftime("%Y-%m-%d %H:%M:%S")
            }
        )
        last_refreshed = gapLastRefreshed.objects.get(id=1)
        end_time = time.time()
